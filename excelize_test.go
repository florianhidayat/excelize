...
func TestSetCellHyperLink(t *testing.T) {
	...
	assert.NoError(t, f.SetCellHyperLink("Sheet1", "B19", "https://github.com/florianhidayat/excelize", "External"))
	// Test add first hyperlink in a work sheet
	assert.NoError(t, f.SetCellHyperLink("Sheet2", "C1", "https://github.com/florianhidayat/excelize", "External"))
	...
	assert.EqualError(t, f.SetCellHyperLink("Sheet1", "A65531", "https://github.com/florianhidayat/excelize", "External"), ErrTotalSheetHyperlinks.Error())
	...
	err = f.SetCellHyperLink("Sheet1", "A1", "https://github.com/florianhidayat/excelize", "External")
	assert.EqualError(t, err, newCellNameToCoordinatesError("A", newInvalidCellNameError("A")).Error())

	// Test update cell hyperlink
	f = NewFile()
	assert.NoError(t, f.SetCellHyperLink("Sheet1", "A1", "https://github.com", "External"))
	assert.NoError(t, f.SetCellHyperLink("Sheet1", "A1", "https://github.com/florianhidayat/excelize", "External"))
	link, target, err := f.GetCellHyperLink("Sheet1", "A1")
	assert.Equal(t, link, true)
	assert.Equal(t, "https://github.com/florianhidayat/excelize", target)
	assert.NoError(t, err)
}
...
func TestGetCellHyperLink(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, link, true)
	assert.Equal(t, target, "https://github.com/florianhidayat/excelize")
...
func prepareTestBook1() (*File, error) {
	...
	if err := f.AddPicture("Sheet1", "F21", filepath.Join("test", "images", "excel.png"),
		&GraphicOptions{
			OffsetX:       10,
			OffsetY:       10,
			Hyperlink:     "https://github.com/florianhidayat/excelize",
